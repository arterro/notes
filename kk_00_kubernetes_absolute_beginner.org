...
*kubelet* is the agent that runs on each node in the cluster making sure that the containers are running on the ndoes as expected.

*Controller Runtime* is the underlying software used to run contaiuners.
...
*Scheduler* is responsible for distributing work / containers across multiple nodes by looking for newly created containers.*Kubeadm*
